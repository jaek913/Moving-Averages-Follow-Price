...
**Calibration of the 50% falsifier.** Because the thesis is falsified by aggregate $S_W < 50\%$, it matters how much genuine attraction is required to drive the metric there. We calibrate this on the decomposition operator itself (`analysis/falsifier_calibration.py`): synthetic log-price paths are generated with an explicit, tunable daily pull toward a trailing mean, and the unmodified decomposition is applied at $H = 63$. Aggregate $S_W$ is monotone decreasing in the injected attraction and crosses 50% at a finite pull (in our parameterization, attraction coefficient $\beta \approx 0.087$): $S_W \approx 100\%$ at zero attraction, $\approx 58\%$ at $\beta = 0.03$, $\approx 52\%$ at $\beta = 0.08$, and $\approx 33\%$ at $\beta = 0.20$. The falsifier is therefore reachable by a sufficiently strong reversion process, not merely rhetorical. Notably, over this same range the realized next-bar toward-rate stays pinned at roughly 64–71% — it barely moves while $S_W$ sweeps from 100% to 33% — which is why we rest the headline on the aggregate share rather than on the toward-rate, and why the direction test is reported separately as the instrument bearing on the *existence* of attraction.
