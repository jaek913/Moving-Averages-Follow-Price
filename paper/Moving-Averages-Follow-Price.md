...
Our empirical validation spans 11 instruments across equities (S&P 500, NASDAQ-100, Nikkei 225, DAX, FTSE 100, Hang Seng), commodities (crude oil, gold), bonds (10-year Treasury notes), and currencies (Euro FX, Japanese Yen), with data histories ranging from 25 to 155 years. We test multiple moving average specifications (SMA, EMA, Hull, with windows from 20 to 200) and apply an extensive validation battery including in-sample/out-of-sample splits, synthetic controls (both i.i.d. and GARCH-calibrated), parameter sensitivity across eight specifications, era analysis, and permutation tests. The result holds, in aggregate, across every dimension we tested.
...
The step-response behavior of causal linear filters — how a filter output converges to a new constant input level — is well-established in the signal processing literature. Oppenheim and Willsky (1997) provide the standard textbook treatment of linear filter theory, including the unity DC gain property that guarantees any properly normalized causal filter will eventually track a constant input. Brown (1963) analyzed exponential smoothing convergence in the context of forecasting. In the financial application of signal processing, Ehlers (2001, Ch. 3) explicitly analyzed the lag and step-function response of SMA and EMA filters after price displacements — plotting the SMA's lag and deriving the EMA's step response — recognizing the lag and convergence behavior as an engineering problem to overcome through filter design (e.g., zero-lag filters). The HMA itself (Hull, 2005) was designed to reduce this lag through a differencing construction that produces negative effective weights.
...
All data are sourced from TradingView export files. The SPX series extends to 1871 using a reconstructed historical index; bars prior to 1926 derive from lower-frequency (monthly) sources, so the early SPX history carries a documented monthly-to-daily frequency transition. We therefore report the pre-1930 era but do not interpret it (3 qualifying events; Section 5.5). A registered ex-1926 decomposition variant (Appendix B.15) re-runs all four SPX cells on data from 1926-01-01 onward: all four remain above 50% (86.2–114.4%), with deltas against the full series between $-3.6$ and $+9.1$ pp and no 50% crossing {{LB-024}}, so the SPX results are not driven by the monthly-reconstruction era. All prices are converted to natural logarithms before analysis; the single non-positive close in the sample (crude oil front-month, 2020-04-20) is dropped prior to the log transform (Appendix B.1).
...
*The primary synthetic null is calibrated with the Hull-50 specification. The eleven SMA-200 cells measured at $H = 63$ violate the $H \geq N$ condition of Theorem 2 (the two windows overlap), so a specification-matched null was run for them (Appendix B.15): under zero attraction the SMA-200 aggregate centers at 100.70% (SE 0.81 pp over 200 zero-drift series), with the 20-sim i.i.d. batch at 100.22% and the GARCH-calibrated controls at 103.4% {{LB-023}} — matching the Hull-50 null and Corollary 1's prediction. The $>100\%$ reading for the SMA-200 cells is therefore benchmarked against a spec-matched null of $\sim$100%.*
...
The paper's most statistically robust empirical finding is that displacement from the moving average predicts elevated forward realized volatility. The Spearman rank correlation between the expanding percentile of displacement magnitude and forward 21-day annualized volatility averages $+0.541$ across all 11 instruments, with a minimum of $+0.448$ and a maximum of $+0.626$. The sign test (11/11 positive) yields $p = 0.00098$. After Bonferroni correction for 38 tests (11 instruments $\times$ 3 correlations each [full-sample, IS, OOS] = 33, plus 5 permutation tests), all 38 remain significant under naive inference that ignores serial dependence {{LB-009}}. Because displacement and forward volatility are both serially persistent, the parametric Spearman $p$-values (below $2 \times 10^{-6}$ on every instrument and sub-sample) are anti-conservative under joint serial dependence; we therefore report corrected inference (Appendix B.15). Under an effective-sample-size correction built from the two series' autocorrelations, the median effective $n$ falls to 82 (from nominal samples of 285–1,181), yet every full-sample corrected $p$ remains at or below $2.1 \times 10^{-5}$. Under circular block permutation preserving serial structure at block lengths of 63 to 504 bars, no shuffled correlation exceeds the real one in any of 5,000 permutations on any rung for any of the five long-history instruments ($p = 0.0002$, the conservative maximum across rungs). In the corrected battery, 33 of 38 tests survive the Bonferroni threshold of $0.05/38 = 0.00132$ — every full-sample and every out-of-sample test; the five non-survivors are all in-sample halves (SPX, NDX, GC, ZN, 6J; corrected $p$ between 0.0017 and 0.0055, each still below 0.05) {{LB-022}}. The robust statement is unchanged: the effect's sign and magnitude (positive on all 11 instruments and all sub-samples, $\rho \approx +0.54$) and now its survival under serial-dependence-corrected inference. The effect is in any case the well-documented persistence of volatility, for which we offer a model-free displacement metric, not a new phenomenon.
...
While the adaptation theorem primarily clarifies what moving averages *do not on their own* tell us (that price is reverting), it also points to what they *can* tell us: displacement magnitude predicts forward volatility on the instruments we tested. This finding ($\rho = +0.54$ on average, positive on all 11 instruments and all sub-samples; 33 of 38 tests — including every full-sample and out-of-sample test — survive Bonferroni correction under the serial-dependence-corrected inference of Section 5.7) has potential applications in volatility forecasting, options pricing, and risk management. Our reading of the theoretical basis: the convergence dynamics implied by the adaptation theorem — regardless of whether convergence occurs through price reversal, average catch-up, or continuation — would tend to produce elevated realized volatility relative to the pre-displacement baseline.
...
More broadly, this paper aims to bring together two literatures that have developed somewhat separately: the macroeconometric tradition recognizing that statistical filters create spurious dynamics (from Slutsky's 1937 demonstration through the modern HP filter critique), and the financial economics tradition interpreting price-MA convergence as evidence of economic mean-reversion. The pieces for this connection have been available for nearly 90 years; we hope the formal decomposition framework presented here offers a useful link between them. The repository regenerates every number on demand; we particularly welcome replications, counterexamples, and extensions to instruments and filters beyond those tested here.
...
All computations were performed in Python 3.12.10 with pandas 2.3.3, numpy 1.26.4, scipy 1.16.3, arch 6.3.0, and openpyxl 3.1.5 (pinned in the repository's `requirements.txt`). Random seeds: 42 for the primary synthetic controls, 20260526 for the 200-series large-sample batteries, 20260610 for the falsifier calibration, and deterministic derived seeds (documented in the script) for the block-permutation battery of B.15. Every number in this paper is regenerated on demand by the committed analysis scripts run on SHA-256-hashed input data; the repository's `claims.lock` ledger and `verify.py` checker enforce this (the checker re-hashes all 36 inputs, re-runs all eleven analysis scripts, compares every load-bearing value within declared tolerances, and is itself validated against a deliberately broken fixture it must reject).
...
### B.15 Post-Certification Follow-Up Battery (Registered)

Three analyses from the adversarial review's deferred-limitations backlog were registered before execution (DESIGN.md amendment of 2026-06-11: scripts committed before first run, decision rules fixed in advance to report the outcome whatever it showed) and executed after the v1.0 certification, entering the paper through the same ledger-and-checker gates as every other number. **(a) Serial-dependence-corrected PE-volatility inference** (`analysis/pe_volatility_blockperm.py`): on the same non-overlapping 21-bar sample as B.6, all 33 Spearman tests receive a Pyper–Peterman effective-sample-size corrected $p$ (autocorrelation lag cap $\min(n/4, 50)$, effective $n$ clipped to $[5, n]$), and the five long-history instruments receive circular block permutations of the PE rank series against fixed forward volatility at block lengths of 3, 6, 12, and 24 samples (63–504 bars), 5,000 permutations per rung with deterministic derived seeds; the reported permutation $p$ is the maximum (most conservative) across rungs. Results: Section 5.7. **(b) Specification-matched SMA-200 synthetic null** (`analysis/synthetic_control_sma200.py`): identical to B.5 in seeds, series lengths, volatility calibration, and GARCH instrument set, with the decomposition specification set to SMA-200. Results: Section 5.3. **(c) Ex-1926 decomposition variant** (`analysis/decomposition_ex1926.py`): the B.3 algorithm on SPX restricted to bars dated 1926-01-01 onward (470 bars dropped), all four MA specifications recomputed from scratch so the expanding threshold and event chain re-form without the monthly-reconstruction era. Results: Section 4.1. The fourth backlog item — roll-window exclusion on the six futures (B.1) — remains future work, tracked in the repository's public corrections log and backlog records.
